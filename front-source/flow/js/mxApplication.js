{
  ...
  const additionAttrSet = new Set(additionAttrs);
      ...
      if (_nodeType !== '') {
        switch (_nodeType) {
          ...
          default:
            ...
            additionAttrs.forEach(nodeName => {
              const val = $.utils[`_${nodeName}`];
              const edit = new mxCellAttributeChange(cell, nodeName, val);
              model.execute(edit);
            });
            break;
        }
  ...
  function customize(mxEditor) {
    ...
    mxEditor.createProperties = function(cell) {
      ...
      if (mxUtils.isNode(value)) {
        ...
        $.utils._knowledgeStrategySwitch = '0';
        $.utils._strategyTriggerCondition = '';
        $.utils._knowledgePlayFlowType = '';
        $.utils._preCallServices = '';
        $.utils._lastCallServices = '';
        ...
        switch (_nodeType) {
          case 'answer':
            ...
            // 创建话术编辑器DOM元素
            $(document).trigger('openDrawer', { type: 'intentionEditor' });
            ...
            for (var i = 0; i < attrs.length; i++) {
              ...
              if (nodeName != 'nodeType' && nodeName != 'artificialIntention') {
                ...
                // 内容
                  ...
                  $(document).trigger('openDrawer', { type: 'speechEditor' });
                }
                // 意图分类级别
                else if (nodeName == 'intentionLevel') {
                ...
                if (additionAttrSet.has(nodeName)) {
                  $.utils[`_${nodeName}`] = val;
                }
              }
            }
            break;
    ...
    mxSvgCanvas2D.prototype.createDiv = function(str, align, valign, style, overflow) {
      ...
      if (!mxClient.IS_IE && document.createElementNS) {
        ...
        return mxUtils.parseXml('<div xmlns="http://www.w3.org/1999/xhtml" style="' + style + '">' + val + '</div>')
          .documentElement;
      }
    };
    ...
    /*mxEditor.setMode = function(modename)
        {
            ...
            else if (modename == 'pan')
            {
                ...
                this.graph.setConnectable(false);com
            }
        };*/
    ...
    mxGraph.prototype.isCellDeletable = function(cell) {
      ...
      if (
        cell.getAttribute('nodeType', '') !== 'start' &&
        typeof selectionCell != 'undefined' &&
        selectionCell.getStyle() !== 'redEdge'
      ) {
    ...
    mxDefaultToolbar.prototype.insert = function(vertex, evt, target) {
      // 通知 topbartool insert
      $(document).trigger('insertVertex', { vertexName: vertex.value.nodeName });
