<template>
    ...
    >
        <ul>
            <li
                    v-for="(item, k) in value.values"
                    v-if="item.speechNodes && item.speechNodes.length"
                    class="speech-node"
                    :class="!(value.isEnd && item.statusName !== STATUS_NORMAL) ? '' : 'hide'"
            >
                <h4>{{item.statusName}}</h4>
                <ul class="speech-list">
                    <li
                            v-for="(node, index) in item.speechNodes"
                            :class="!(value.isEnd && index > 0) ? '' : 'hide'"
                    >
                        <span
                                class="speech-rounds"
                        ...
                        >
                            ...
                            <TipsControl
                                    ...
                                    class="tips-control"
                                    :showDeleteButton="item.speechNodes.length > MIN_SPEECH_NODE && !value.values[k].disabled && !value.isEnd"
                                    :showAppendButton="item.speechNodes.length < MAX_SPEECH_NODE && !(value.isRoot && k <= 0) && !value.values[k].disabled && !value.isEnd"
...
<script>
    ...
    import { MAX_SPEECH_NODE, MIN_SPEECH_NODE, STATUS_NORMAL} from '../../const';
    ...
    export default {
        ...
        methods: {
            ...
            speechRichEditorFocus(index) {
                if (this.focusEditorTime) {
                    clearTimeout(this.focusEditorTime);
                }
                this.focusEditorTime = setTimeout(() => {
