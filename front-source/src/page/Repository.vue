  ...
  <div :class="{'sub-collapse':isSubCollapse}">
    <div class="rep-box-slider">
      <div class="repository-menu-box">
        <div class="repository-menu-top">
          <a @click="handleManageReposClick" class="header-button">
            <i class="el-icon-folder-opened"></i>知识库模版
          </a>
          <div class="header-title">
            知识库行业分类
            <i class="iconfont icon-ico_add" @click="addGroup"></i>
          </div>
        </div>
          ...
          <draggable v-model="repositoryMenu" @end="draggableEnd" @start="draggableStart">
            <transition-group>
              <div
                ...
                v-for="(item,index)  in repositoryMenu"
                :class="{active:index==itemActive,edit:item.isEdit}"
                :key="item.id || `add-${index}`"
              >
                <div class="repository-menu-input-box" v-show="item.show">
                  <el-input v-model="item.typeName" v-if="item.type!=1&&item.type!=2"></el-input>
                </div>

                <span
                  v-show="!item.show"
                  class="groupName"
                  @click="queryKnowledgeGroupListOfPage(item,index,'handleClick')"
                >{{item.typeName}}</span>
    ...
    <div class="table">
      ...
      <el-dialog title="删除确认" :visible.sync="removeTypeVisible" width="400px">
        <div>
          您确定要删除知识库分类“{{removeTypeName}}”吗？
          <br />相关知识库将不会被删除，继续保留在“未分类”中
        </div>
        <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
        ...
        if (res.resultMessageEnum == '0000') {
          if (res.returnObject && res.returnObject.length) {
            ...
            if (this.itemActive >= 0) {
              if (res.returnObject[this.itemActive]) {
                id = res.returnObject[this.itemActive].id;
              }
            }
            ...
            if (this.$router.history.current.fullPath.indexOf('/sub/') < 0) {
              this.$router.push(`/repository/set/${id}`);
            }
          } else {
            this.$router.push(`/repository/set/unclassified`);
      ...
      $.post(this.$baseUrl + 'common/checkAnnexFile.json', JSON.stringify(data)).then(res => {
        if (res.resultMessageEnum == '0000') {
          ...
          if (!res.returnObject.correctFlag) {
            let msg = `${res.returnObject.rowNumber - res.returnObject.exceptionRowNumber}条上传成功，剩余${
              res.returnObject.exceptionRowNumber
            }条上传失败！`;
    ...
    handleManageReposClick() {
      bus.$emit('manangeReposVisible');
    },
  },
  watch: {
    addGroupVisible(val) {
