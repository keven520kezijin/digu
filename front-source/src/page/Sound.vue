<template>
    <div class="sound-page" :class="{'soundPath':isSound}" >
                ...
                <div slot="header" class="clearfix">
                    <el-row>
                        ...
                        <el-col :span="10" class="sound-card-right">
                            ...
                            <el-button type="primary" :disabled="del_disabled"  class="upload-btn-sound">替换
                                <input multiple="multiple" :disabled="del_disabled" ref="upload_input" type="file" id="file1" @change="multithMatchUpLoadCheck" >
                            </el-button>
                            <el-button type="primary" icon="delete" class="sound_top_btn_del" :disabled="del_disabled" @click="showRemoveDialog">删除</el-button>
                        </el-col>
                    </el-row>
                </div>
                        ...
                        <el-col :span="19">
                            <el-table stripe :data="configGroupIdData" 
                                ...
                                <el-table-column label="操作" prop="" width="60" v-if="!isSound">
                                    <template slot-scope="scope">
                                        <el-button type="text" @click="handleUse(scope.row)">使用</el-button>
                                    </template>
                                </el-table-column>
       ...
       <!--  删除知识库分类 -->
        ...
        <!--覆盖音频名 -->
        <el-dialog title="覆盖音频名" :visible.sync="replaceUniqueNameVisible" width="400px" :append-to-body="true">
            <div >替换音频名称不一致,上传后会以音频库里原有音频名进行覆盖</div>
            <span slot="footer" class="dialog-footer">
                <el-button @click="replaceUniqueNameVisible = false">取 消</el-button>
                <el-button type="primary" @click="multithMatchUpLoad">确 定</el-button>
            </span>
        </el-dialog>
        <!--无法替换-->
        <el-dialog title="无法替换" :visible.sync="unableReplaceVisible"  width="400px"  :append-to-body="true">
            <div >抱歉，部分音频正在被外呼任务使用中，请任务结束后再进行替换</div>
            <span slot="footer" class="dialog-footer">
                <el-button type="primary" @click="unableReplaceVisible = false">确 定</el-button>
            </span>
        </el-dialog>
        <!-- 批量替换-->
        <el-dialog title="替换音频" :visible.sync="reVisible" :append-to-body="true">
            <span style="color: #409EFF">
                已从本地选择<span style="color:#E6A23C">{{localSelectCount}}条</span>音频对
                <span style="color: #409EFF">{{selectMatchCount}}条</span>目标音频进行替换
            </span>

            <el-table

                    :data="selectMatchSoundList"
                    style="width: 100%"
                    :key="Math.random()"
            >
                <el-table-column
                ...
                >
                    <template slot-scope="scope">
                        <span v-if="scope.row.matchResult" style="color: #00d1b2">成功</span>
                        <span v-if="!scope.row.matchResult" style="color: red">失败，音频名称不匹配</span>
                    </template>
            ...
            <span slot="footer" class="dialog-footer">
                <el-button @click="reVisible = false">重新选择</el-button>
                <el-button type="primary" @click="hasTaskRuningBySoundList">确认替换</el-button>
            </span>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                sortList:[],
                selectable:false,
                replaceUniqueNameVisible:false,
                unableReplaceVisible:false,
                reVisible:false,
                selectMatchSoundList:[],
                selectMatchCount:0,
                localSelectCount:0,
                selectAndMatchSoundPathArr:[],
                selectAndMatchFileName:[],
                activeGroupId:'',
            }
        },
        created() {
        ...
        methods: {
            ...
            showRemoveDialog(){
                ...
                this.multipleSelection.every(item => {
                    if (item.useFlag !== 0){
                        this.$message.error('【' + item.fileName + '】音频已被使用，不能删除！');
                        isUse = true;
                        return false;
                    } ;
                });
            ...
            //批量匹配
            multithMatchUpLoad(){
                this.replaceUniqueNameVisible = false
                let self = this;
                var oMyForm = new FormData();
                ...
                for(var i=0;i<files.length;i++){
                    ...
                    if(this.selectAndMatchFileName.indexOf(files[i].name)>=0 || this.selectMatchSoundList.length == 1 ){
                        oMyForm.append("file",files[i]);
                    }
                }
                var matchList = []
                for (var i in this.selectMatchSoundList){
                    if(this.selectMatchSoundList[i].matchResult || this.selectMatchSoundList.length == 1){
                        let item ={
                            id:this.selectMatchSoundList[i].id,
                            soundPath:this.selectMatchSoundList[i].soundPath,
                            fileName:this.selectMatchSoundList[i].fileName
                        };
                        matchList.push(JSON.stringify(item));
                    }
                }
                //加一个后台分割的标示
                oMyForm.append("matchList",matchList.join("|||"));
                console.log("this.selectMatchSoundList:",matchList);
                $.ajax({
                    url: this.$baseUrl+'sound/uploadAndReplaceSoundFileList.json',
                    type: 'POST',
                    cache: false,
                    data:oMyForm,
                    dataType: "json",
                ...
                console.log(oMyForm)
                this.changeFileType1()
            },
            //批量匹配检测
            multithMatchUpLoadCheck(){
                let selectNameArr = [] //前端选择的音频名称数组
                let selectSoundPathArr = [] //前端选择的音频路径数组
                let uploadNameArr = [] //上传的文件名称数组
                let files = $('#file1')[0].files;
                this.localSelectCount = 0
                this.selectMatchCount = 0
                this.selectMatchSoundList = []
                this.selectMatchSoundList = this.multipleSelection
                this.selectAndMatchSoundPathArr = []
                this.selectAndMatchFileName = []
                this.localSelectCount = files.length;
                for (let i in this.multipleSelection){
                    selectNameArr.push(this.multipleSelection[i].fileName)
                    selectSoundPathArr.push(this.multipleSelection[i].soundPath)
                }

                for(var i=0;i<files.length;i++){
                    uploadNameArr.push(files[i].name)
                }
                //去除重复的select fileName
                selectNameArr = this.uniq(selectNameArr);
                selectSoundPathArr = this.uniq(selectSoundPathArr)

                //判断 :单选音频进行替换需名称一致，若名称不一致，以线上音频名称为准，提示：“上传后会以音频库里原有音频名进行覆盖” 确认|取消
                //如果 选择和上传的数量一直，但是名称不一致
                if (selectNameArr.length == 1 && uploadNameArr.length == 1 && selectNameArr[0]!=uploadNameArr[0]){
                    this.replaceUniqueNameVisible = true
                    return;
                }

                //选择的文件少于上传的文件
                if(selectNameArr.length == 1 && uploadNameArr.length > 1 ){
                    this.$message.error("上传文件太多，请重新选择")
                    return;
                }

                //批量替换
                if (this.selectMatchSoundList.length > 1){
                    for (var i in this.selectMatchSoundList){
                        if (uploadNameArr.indexOf(this.selectMatchSoundList[i].fileName) >= 0){
                            this.selectMatchSoundList[i].matchResult = true
                            this.selectMatchCount += 1;
                            this.selectAndMatchSoundPathArr.push(this.selectMatchSoundList[i].soundPath)
                            this.selectAndMatchFileName.push(this.selectMatchSoundList[i].fileName)
                        }else {
                            this.selectMatchSoundList[i].matchResult = false
                        }
                    }
                    this.reVisible = true
                    return
                ...
                return this.multithMatchUpLoad();
            },
            hasTaskRuningBySoundList:function(){
                this.reVisible = false;
                if(this.selectAndMatchSoundPathArr.length <= 0){
                    this.$message.error("没有任何匹配，替换失败")
                    return;
                }
                $.post(this.$baseUrl+'task/hasTaskRuningBySoundList.json',JSON.stringify(this.selectAndMatchSoundPathArr)).then((res)=>{
                    if(res.resultMessageEnum=='0000'){
                        //如果有弹出框
                        if (res.returnObject){
                            this.unableReplaceVisible = true
                            return;
                        }else{
                            this.multithMatchUpLoad()
                        }
                    }else{
                        this.$message.error(res.errorInfoList[0].errorMessage);
                        return
