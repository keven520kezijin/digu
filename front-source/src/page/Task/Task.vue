<template>
  <div class="empty-div" v-if="isEmpty">
    ...
    <p>
      请先去
      <a href="/index.html#/speech">我的话术</a>创建“智能外呼方案”类型的话术模版，才能使用创建任务哦
    </p>
  </div>
  <div v-else class="table table-main">
        ...
        >
          ...
          <el-table-column prop="taskStatus" label="进度">
            <template slot-scope="scope">
              ...
              <div v-else>
                <span v-bind:id="scope.row.id" class="progress-text"
                  >{{ scope.row.calledPhoneNumber }}/{{ scope.row.phoneNumber }}</span
                >
                <el-progress
                  :text-inside="true"
                  :stroke-width="18"
                  :status="scope.row.status || 'success'"
                  :percentage="scope.row.scale || 0"
                ></el-progress>
              </div>
            </template>
          </el-table-column>
            ...
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.taskStatus != 4">
                <el-button
                  ...
                  @click="handleStartTask(scope.$index, scope.row)"
                  >开始</el-button
                >
                <el-button
                  type="text"
                  v-show="scope.row.taskStatus == 2"
                  @click="handleStopTask(scope.$index, scope.row)"
                  >取消</el-button
                >
                <el-button
                  type="text"
                  v-show="scope.row.taskStatus == 2"
                  @click="handlePauseTask(scope.$index, scope.row)"
                  >暂停</el-button
                >
                <el-button
                  type="text"
                  v-show="scope.row.taskStatus == 8"
                  @click="handleResumeTask(scope.$index, scope.row)"
                  >继续</el-button
                >
                <el-button type="text" v-show="scope.row.taskStatus == 1" @click="showDeleteDialog(scope.row)"
                  >删除</el-button
                >
                <el-button type="text" v-show="scope.row.taskStatus == 1" @click="adapterServiceEdit = scope.row"
                  >调用服务</el-button
                >
    ...
    <el-dialog title="新建任务" class="creatDialog" :visible.sync="cretVisible" width="800px">
      ...
      >
        <el-form-item label="任务名称" prop="taskName">
          <el-input placeholder="输入任务名称" v-model="creatForm.taskName"></el-input>
        </el-form-item>
        <el-form-item prop="speechcraftName" label="话术名称">
          <el-select
            v-model="creatForm.speechcraftName"
            placeholder="选择我的话术库"
            style="width:380px;"
            @change="changeValue"
          >
        ...
        <el-form-item prop="taskLevel" label="优先级">
          <el-select
            v-model="creatForm.taskLevel"
            value-key="valueName"
            placeholder="选择任务优先级"
            style="width:380px;"
          >
            <el-option
              v-for="item in taskLevels"
              :key="item.systemTypeCode"
              :label="item.valueName"
              :value="item.systemValue"
            ></el-option>
          </el-select>
          ...
          <el-col :span="4" class="data-btn-box">
            ...
            <div id="task_upload_box">
              <el-button class="file-btn" @click.native="handleFile">
                <input
                  v-show="creatForm.speechcraftId"
                  ref="upload_input"
                  type="file"
                  id="file"
                  @change="uploadAnnexFile"
                />
        ...
        <div class="el-form--label-top adapter-field-container">
          <adapter-fields v-model="creatForm"></adapter-fields>
        </div>
      </el-form>
    ...
    <!-- 服务调用 -->
    <el-dialog title="服务调用" :visible="isAdapterServiceEditVisible" @close="handleASEditClose">
      <adapter-form
        v-if="isAdapterServiceEditVisible"
        @close="handleASEditClose"
        @refresh="queryTaskList"
        :task="adapterServiceEdit"
      ></adapter-form>
    </el-dialog>
  </div>
</template>
...
import AdapterFields from '../../components/AdapterFields';
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        speechcraftName: [{ required: true, message: '请选择话术模版！', trigger: 'change' }],
        taskName: [{ required: true, message: '任务名称不能为空！', trigger: 'change' }],
        callerPhone: [{ required: true, message: '请选择外显号码！', trigger: 'change' }],
        // filePath:[
  ...
  async created() {
    ...
    await Promise.all([this.queryTaskList(), this.queryMemberDetail(), this.querySpeechcraftList()]);
  },

  computed: {
    isAdapterServiceEditVisible() {
      return this.adapterServiceEdit !== null;
    },
  },
  ...
  methods: {
    ...
    changeValue(vId) {
      ...
      this.downTaskItemExcelTemplateHref =
        this.$baseUrl + `task/downTaskItemExcelTemplate.html?speechcraftId=${this.creatForm.speechcraftId}`;
    ...
    creatHandleTask(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          const speech = find(this.creatOptions, { id: this.creatForm.speechcraftId });
          const { preCallServiceIds, lastCallServiceIds } = this.creatForm;
          let data = {
            ...
            nluAppId: speech ? speech.nluAppId : '',
            nluAppKey: speech ? speech.nluAppKey : '',
            callServiceSwitch: this.creatForm.callServiceSwitch ? 1 : 0,
            preCallServiceIds: preCallServiceIds && preCallServiceIds[preCallServiceIds.length - 1],
            lastCallServiceIds: lastCallServiceIds && lastCallServiceIds[lastCallServiceIds.length - 1],
          };
