      ...
      <el-dialog title="呼叫上限设置" :visible.sync="dialogFormVisible">
        <el-form :model="form" :label-position="labelPosition" label-width="120px">
          ...
          <el-form-item label="关机">
            ...
            <el-input-number
              v-if="closedSelect == 'custom'"
              v-model="form.callLimitClosed"
          ...
          <el-form-item label="停机">
            ...
            <el-input-number
              v-if="downtimeSelect == 'custom'"
              v-model="form.callLimitDowntime"
              controls-position="right"
              :min="1"
              :max="1000"
            ></el-input-number>
          ...
          <el-form-item label="用户忙">
            ...
            <el-input-number
              v-if="refusedSelect == 'custom'"
              v-model="form.callLimitRefused"
              controls-position="right"
              :min="1"
              :max="1000"
            ></el-input-number>
          ...
          <el-form-item label="无应答">
            ...
            <el-input-number
              v-if="noAnswerSelect == 'custom'"
              v-model="form.callLimitNoAnswer"
              controls-position="right"
              :min="1"
              :max="1000"
            ></el-input-number>
          ...
          <el-form-item label="线路故障">
            <el-select v-model="limitBreakDownSelect">
              <el-option label="不限" value="unlimited"></el-option>
              <el-option label="自定义" value="custom"></el-option>
            </el-select>
            <el-input-number
              v-if="limitBreakDownSelect == 'custom'"
              v-model="form.callLimitBreakDown"
              controls-position="right"
              :min="1"
              :max="1000"
            ></el-input-number>
          ...
          <el-form-item label="空号">
            ...
            <el-input-number
              v-if="emptySelect == 'custom'"
              v-model="form.callLimitEmpty"
              controls-position="right"
              :min="1"
              :max="1000"
            ></el-input-number>
          ...
          <el-form-item label="呼叫失败">
            ...
            <el-input-number
              v-if="callFailSelect == 'custom'"
              v-model="form.callLimitCallFail"
              controls-position="right"
              :min="1"
              :max="1000"
            ></el-input-number>
...
export default {
  ...
  data() {
    return {
      ...
      limitBreakDownSelect: 'unlimited',
    };
  },
  created() {
  ...
  methods: {
    exportNumber() {
      ...
      let params = ids;
      $.post(this.$baseUrl + 'task/exportInvalidNumberListFile.json', JSON.stringify(params)).then(res => {
    ...
    queryInvalidNumberList() {
      $.post(this.$baseUrl + 'task/queryInvalidNumberList.json', JSON.stringify(this.queryPageCondition)).then(res => {
        if (res.resultMessageEnum == '0000') {
      ...
      $.get(this.$baseUrl + 'task/queryTaskConfig.json').then(res => {
        if (res.resultMessageEnum == '0000') {
          if (res.returnObject) {
            ...
            if (taskConfig.callLimitClosed >= 0) {
              this.closedSelect = 'custom';
              this.form.callLimitClosed = taskConfig.callLimitClosed;
            } else {
              this.closedSelect = 'unlimited';
            }
            if (taskConfig.callLimitDowntime >= 0) {
              this.downtimeSelect = 'custom';
              this.form.callLimitDowntime = taskConfig.callLimitDowntime;
            } else {
              this.downtimeSelect = 'unlimited';
            }
            if (taskConfig.callLimitRefused >= 0) {
              this.refusedSelect = 'custom';
              this.form.callLimitRefused = taskConfig.callLimitRefused;
            } else {
              this.refusedSelect = 'unlimited';
            }
            if (taskConfig.callLimitNoAnswer >= 0) {
              this.noAnswerSelect = 'custom';
              this.form.callLimitNoAnswer = taskConfig.callLimitNoAnswer;
            } else {
              this.noAnswerSelect = 'unlimited';
            }
            if (taskConfig.callLimitBreakDown >= 0) {
              this.limitBreakDownSelect = 'custom';
              this.form.callLimitBreakDown = taskConfig.callLimitBreakDown;
            } else {
              this.limitBreakDownSelect = 'unlimited';
            }
            if (taskConfig.callLimitCallFail >= 0) {
              this.callFailSelect = 'custom';
              this.form.callLimitCallFail = taskConfig.callLimitCallFail;
            } else {
              this.callFailSelect = 'unlimited';
            }
            if (taskConfig.callLimitEmpty >= 0) {
              this.emptySelect = 'custom';
              this.form.callLimitEmpty = taskConfig.callLimitEmpty;
            } else {
              this.emptySelect = 'unlimited';
            }
            if (taskConfig.callLimitForword >= 0) {
              this.forwordSelect = 'custom';
              this.form.callLimitForword = taskConfig.callLimitForword;
            } else {
              this.forwordSelect = 'unlimited';
            }
          } else {
          }
        } else {
    ...
    submit() {
      if (this.closedSelect == 'unlimited') {
        this.form.callLimitClosed = -1;
      }
      if (this.downtimeSelect == 'unlimited') {
        this.form.callLimitDowntime = -1;
      }
      if (this.refusedSelect == 'unlimited') {
        this.form.callLimitRefused = -1;
      }
      if (this.noAnswerSelect == 'unlimited') {
        this.form.callLimitNoAnswer = -1;
      }
      if (this.callFailSelect == 'unlimited') {
        this.form.callLimitCallFail = -1;
      }
      if (this.emptySelect == 'unlimited') {
        this.form.callLimitEmpty = -1;
      }
      if (this.forwordSelect == 'unlimited') {
        this.form.callLimitForword = -1;
      }
