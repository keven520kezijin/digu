...
// 意向级别可选项
const _defaultIntentionLevelOptions =
  '意图分类级别可以帮助您对对话术模板中的每个意图进行标记并归类，便于话术流程管理。';
  ...
  $.extend($.utils, {
      ...
      $(xmlDoc)
        ...
        .each(function() {
          if (typeof $(this).attr('speechcraft') != 'undefined' && $(this).attr('speechcraft') != '') {
            ...
            xml = xml.replace(
              JSON.stringify(speechcraft).replace(/\"/g, '&quot;'),
          ...
          unlockSpeechCont: function() {
            $.utils._speechcraftContent[this.flowTypeName].forEach(elem => {
              if (elem.name === this.editableTabsName) {
                ...
                $('#_' + this.flowTypeName + '-' + this.editableTabsName)
                  .parent()
              ...
              if (pathIndex != '-1' && contentIndex != '-1') {
                ...
                leftKH.forEach(el => {
                  ...
                  if (leftKHSplit.length == 2) {
                    ...
                    let inputStr =
                      '[' + leftKHSplit[0] + ']' + '<input type="text" id="' + leftKHSplit[0] + '" value="">';
      ...
      return new Vue({
        ...
        methods: {
          ...
          saveKnowledge() {
            ...
            $.ajax({
              type: 'POST',
              url: _baseUrl + '/manage/services/speechcraft/modifySpeechcraft.json',
              dataType: 'json',
          ...
          querySpeechcraftDetail() {
            console.log( _baseUrl + '/manage/services/speechcraft/querySpeechcraftDetail.json')
            $.ajax({
              type: 'GET',
              url: _baseUrl + '/manage/services/speechcraft/querySpeechcraftDetail.json?speechcraftId=' + speechcraftId,
              dataType: 'json',
              contentType: 'application/json',
              async: false,
            }).then(res => {
              if (res.resultMessageEnum == '0000') {
                ...
                if (data.useKnowledgeFlag === 1) {
                  $.utils._dialogVue.knowledgeForm.singleAskedLimit =
                    typeof data.singleAskedLimit === 'undefined' ? 1 : data.singleAskedLimit;
                  $.utils._dialogVue.knowledgeForm.hungupSpeechcraftContent =
                    typeof data.hungupSpeechcraftContent === 'undefined' ? '' : data.hungupSpeechcraftContent;
                  if ($.utils._dialogVue.knowledgeVisible) {
                  ...
                  if (knowledgeList != null) {
                    ...
                    knowledgeList.forEach((item, index) => {
                      $.utils._dialogVue.knowledgeForm.knowledge.push({
                        knowledgeGroupId: item.knowledgeGroupId,
                        knowledgeGroupName: item.knowledgeGroupName,
                      });
                    });
                  }
                }
          ...
          queryKnowledgeGroupList() {
            $.get(_baseUrl + '/manage/services/knowledge/queryKnowledgeGroupList.json?memberId=' + memberId).then(
              res => {
                if (res.resultMessageEnum == '0000') {
                  let data = res.returnObject;
                  if (data != null) {
                    ...
                    data.forEach((item, index) => {
                      $.utils._dialogVue.knowledgeList.push({
                        knowledgeGroupId: item.id,
                        knowledgeGroupName: item.groupName,
                      });
                    });
                  }
                } else if (res.resultMessageEnum == '0001') {
          ...
          queryAllIndustryList() {
            $.get(_baseUrl + '/manage/services/intention/queryAllIndustryList.json').then(res => {
              if (res.resultMessageEnum == '0000') {
          ...
          saveIntentLevelDesc() {
            ...
            $.ajax({
              type: 'POST',
              url: _baseUrl + '/manage/services/speechcraft/modifyIntentionLevelDescription.json',
              dataType: 'json',
              contentType: 'application/json',
            ...
            this.$refs[formName].validate(valid => {
              ...
              if (valid) {
                let commonSpeechcraftVO = {
                  ...
                  artificialIntention:
                    this.speechForm.artificialIntention != '' ? this.speechForm.artificialIntention.join(',') : '',
                };
                $.ajax({
                  type: 'POST',
                  url: _baseUrl + '/manage/services/speechcraft/modifySpeechcraft.json',
          ...
          uploadRecord(id) {
            ...
            uploadFile.forEach((elem, index) => {
              if (elem.$refs.input.files.length === 1) file = elem.$refs.input.files[0];
            });
            formData.append('sourceType', 103);
            formData.append('file', file);
            $.ajax({
              url: _baseUrl + '/manage/services/common/uploadAnnexFile.json',
              type: 'POST',
              cache: false,
              data: formData,
              dataType: 'json',
          ...
          downloadRecord(id) {
            ...
            window.location.href = _baseUrl + '/manage/services/file/downloadFile.html?filePath=' + filePath;
          ...
          exit() {
            ...
            if (_baseUrl === 'http://127.0.0.1:8083') {
              window.location.href = _baseUrl + '/index.html#/speech?type=2';
            } else {
              window.location.href = _baseUrl + '/#/speech?type=2';
            }
          },
          /**
           * 保存并退出
          ...
          handlePlaying() {
              ...
              for (var i in this.splitAuditionData.audios) {
                if (!this.splitAuditionData.audios[i].hasOwnProperty('url')) {
                  if (this.splitAuditionData.audios[i].text.indexOf(el) != -1) {
                    this.splitAuditionData.audios[i].text = this.splitAuditionData.audios[i].text.replace(
                      '[' + el + ']',
                      elValue,
                    );
                  }
            ...
            $.ajax({
              type: 'POST',
              url: _baseUrl + '/manage/services/speechcraft/testListen.json',
              dataType: 'json',
              contentType: 'application/json',
              ...
              success: function(data) {
                ...
                if (data.resultMessageEnum == '0000') {
                  self.viewSource =
                    _baseUrl + '/manage/services/file/downloadFile.html?filePath=' + data.returnObject.joinUrl;
          ...
          percentMusic() {
            ...
            this.$refs.w_audio.ontimeupdate = () => {
              ...
              var value =
                this.currentMusic.currentTime && duration ? (this.currentMusic.currentTime / duration) * 100 : 0;
          ...
          changeSlider(time) {
            if (this.$refs.w_audio) {
              ...
              this.currentMusic.currentTime = this.$refs.w_audio.currentTime =
                (time * this.$refs.w_audio.duration) / 100 || 0;
          ...
          exceptionFunc: function() {
            ...
            try {
              ...
              let _overlay = new mxCellOverlay(new mxImage('images/undo.gif', 16, 16), '', 'right', 'top');
              let _overlay1 = new mxCellOverlay(new mxImage('images/undo.gif', 16, 16), '', 'right', 'top');
      ...
      return new Vue({
        ...
        methods: {
          ...
          queryIntentionTemplateList: function() {
            ...
            $.ajax({
              type: 'POST',
              url: _baseUrl + '/manage/services/intention/queryIntentionSpeechcraftList.json',
              dataType: 'json',
              contentType: 'application/json',
    ...
    queryCurrentSpeechcraftDetail() {
      $.ajax({
        ...
        if (res.resultMessageEnum == '0000') {
          let data = res.returnObject;
          var intentionLevelDescriptionStr = data.intentionLevelDescription;
          if (typeof intentionLevelDescriptionStr != 'undefined' && intentionLevelDescriptionStr != '') {
            intentionLevelDescriptionObj = JSON.parse(intentionLevelDescriptionStr);
