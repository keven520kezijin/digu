<template>
  ...
  <div class="container-admin">
      ...
      <el-row class="first-row" v-if="this.ypkActiveName == 'ypkFirst'">
        ...
        <el-col>
          ...
          <div style="float: right">
            <el-button type="primary" @click="dialogPublicGroupVisible = true" :disabled="this.publicDisabled == false"
              >发布</el-button
            >
          </div>
        </el-col>
      ...
      <el-row v-if="this.ypkActiveName == 'ypkFirst'">
        <el-col :span="24" class="repository-menu-box">
          <el-tabs tab-position="left" @tab-click="selectLY">
            <el-tab-pane v-for="item in industryListaddQb" :label="item.industryName" v-bind:key="item.id">
              <el-col>
                <el-table
                  stripe
                  :data="listOfPageData"
                ...
                >
                  ...
                  <el-table-column width="250" sortable prop="editTime" label="更新时间"></el-table-column>
    ...
    <el-dialog title="删除知识库" :visible.sync="dialogDelGroupVisible" width="25%">
    ...
    <!--发布知识库-->
    <el-dialog title="发布知识库" :visible.sync="dialogPublicGroupVisible" width="25%">
...
export default {
  ...
  methods: {
        ...
        if (res.resultMessageEnum == '0000') {
          if (res.returnObject && res.returnObject.length) {
            ...
            for (let i in this.industryList) {
              this.industryListaddQb.push(this.industryList[i]);
            }
          }
        } else {
    ...
    queryKnowledgeGroupListOfPage: function() {
      ...
      params.searchType = this.searchForm.insdutry ? '2' : '0';
      ...
      if (this.searchForm.insdutry != '') {
        params.knowledgeTypeId = this.searchForm.insdutry;
      }

      $.post(this.$baseUrl + 'services/knowledge/queryKnowledgeGroupListOfPage.json?', JSON.stringify(params)).then(
        res => {
          if (res.resultMessageEnum == '0000') {
    ...
    publishKnowledgeGroupAll() {
      ...
      $.post(this.$baseUrl + 'services/knowledge/publishKnowledgeGroupBatch.json?', JSON.stringify(params)).then(
        res => {
          if (res.resultMessageEnum == '0000') {
