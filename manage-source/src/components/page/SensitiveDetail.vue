  ...
  <div class="container-admin">
    ...
    <div class="admin-body">
      <el-row class="first-row">
        <el-col>
          <el-form :inline="true" class="w-form-inline">
            <el-form-item label="关键词：">
              <el-input clearable v-model="searchForm.queryKey" placeholder="敏感词库名称" @keyup.native.13="onSearch">
              </el-input>
            </el-form-item>
            <el-form-item label="操作人：">
              <el-select v-model="searchForm.memberIds" multiple placeholder="请选择">
                <el-option v-for="item in managerList" :key="item.id" :label="item.loginName" :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item>
        ...
        <el-col>
          <el-table :data="dataList" @selection-change="handleSelectionChange" @cell-click="enterModify">
                ...
                <span :id="row.id" style="display: none">
                  ...
                  <span>
                    <el-button
                      type="text"
                      size="small"
                      @mouseout.native="cancelChangeLibraryNameOut"
                      @mouseover.native="cancelChangeLibraryNameOver"
                      @click="cancelChangeLibraryName"
                      >取消</el-button
                    >
                    <el-button
                      type="text"
                      size="small"
                      @mouseout.native="confirmChangeLibraryNameOut"
                      @mouseover.native="confirmChangeLibraryNameOver"
                      @click="confirmChangeLibraryName(row)"
                      >确定</el-button
                    >
                  </span>
                </span>
            ...
            <el-table-column sortable prop="editTime" label="更新时间"> </el-table-column>

            <el-table-column prop="editor" label="操作人"> </el-table-column>
        ...
        <el-col>
          <el-dialog title="新增敏感词" :visible.sync="addNewEntry" width="30%">
          ...
          <el-dialog title="删除敏感词" :visible.sync="delVisible" width="30%">
          ...
          <el-dialog title="批量上传敏感词" :visible.sync="batchUploadVisible" width="30%">
            ...
            <span style="font-size: smaller;color: red;padding-left: 110px;"
              >注:批量导入的格式为TXT文件,每行一个敏感词,文件大小不超多5M</span
            >
            <span slot="footer" class="dialog-footer">
...
export default {
  data() {
    return {
      ...
      rules1: {
        file1: [{ required: true, message: '请输选择文件', trigger: 'blur' }],
      },
      multipleSelection: [],
