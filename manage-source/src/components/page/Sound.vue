  ...
  <div class="sound-page" :class="{'soundPath':isSound}">
          ...
          <el-row>
            ...
            <el-col :span="10" class="sound-card-right">
              ...
              <el-button
                type="primary"
                :disabled="disabled"
                @click="createSoundConfigItemExcelFile"
              >导出表格</el-button>
              <el-button
                type="primary"
                :disabled="disabled"
                class="upload-btn-sound"
                @click="uploadBtn"
              >上传表格</el-button>
              <el-button
                type="primary"
                icon="delete"
                class="sound_top_btn_del"
        ...
        <div class="repository-box">
          <el-row>
            <el-col :span="5" class="repository-menu-box" :style="{height:height+72+'px'}">
              <div class="repository-menu-top">
                文件夹
                <i class="el-icon-circle-plus" @click="addGroup"></i>
              </div>
              <div class="repository-menu sound-menu">
                ...
                >
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          v-show="!item.show"
                          class="groupName"
                          @click="querySoundConfigItemList(item,index)"
                        >{{item.configGroupName}}</span>
                        ...
                        <div
                          class="repository-icon-box hover-box"
                          v-if="item.groupFlag!=1&&item.groupFlag!=0"
                        >
                          <i class="iconfont icon-ico_move-sort"></i>
                          <i
                            class="iconfont icon-ico_edit-content"
                            @click="editConfigGroupMenu(item,'speech')"
                          ></i>
                  ...
                  <el-submenu index="2" class="repository-menu">
                    ...
                    <draggable
                      v-model="knowledgeMenu"
                      @end="draggableEnd"
                      @start="draggableStart('knowledge')"
                    >
                      <el-menu-item
                        :index="`2-${index}`"
                        class="repository-menu-item"
                      ...
                      >
                        ...
                        >
                          ...
                          <i
                            class="iconfont icon-ico_edit-content"
                            @click="editConfigGroupMenu(item,'knowledge')"
                          ></i>
                          <i class="iconfont icon-ico_delete" @click="deletSound(item,index)"></i>
                        </div>
                      </el-menu-item>
                  ...
                  <template slot-scope="scope">
                    <div class="video-edit-box">
                      ...
                      <el-input
                        ...
                        v-if="!scope.row.edit"
                        placeholder="输入文本内容"
                        v-model="scope.row.soundContent"
    ...
    <el-dialog title="删除确认" :visible.sync="removeSoundVisible" width="400px" :append-to-body="true">
      <div>
        您确定要删除音频管理库分类
        <span
          v-show="removeSoundInfo.configGroupName"
        >“{{removeSoundInfo.configGroupName}}”</span>吗？删除后相关音频也会被删除
      </div>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="上传确认" :visible.sync="uploadVisible" width="400px" :append-to-body="true">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" class="upload-btn-sound">
          确 定
          <input
            type="file"
            :disabled="disabled"
            id="importFile"
            @change="importSoundConfigItemExcelFile"
          />
        </el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    removeSoundConfigGroup() {
      ...
      $.post(this.$baseUrl + 'sound/removeSoundConfigGroup.json?soundConfigGroupId=' + data.soundConfigGroupId).then(
        res => {
          if (res.resultMessageEnum == '0000') {
    ...
    createSoundConfigItemExcelFile() {
      ...
      $.post(this.$baseUrl + 'services/file/createSoundConfigItemExcelFile.json', JSON.stringify(data)).then(res => {
        if (res.resultMessageEnum == '0000') {
