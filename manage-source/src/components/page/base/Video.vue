...
export default {
    ...
    methods: {
        ...
        //得到文件播放时长
        getFilePlayDuration(){
            return $.get(this.$baseUrl+'services/file/getFilePlayDuration.json?filePath='+encodeURIComponent(this.soundPath)).then((res)=>{
    ...
    watch:{
        source(src){
            ...
            // this.viewSource = src;
            this.soundPath = src;
            this.viewSource = this.$baseUrl+'services/file/downloadFile.html?filePath='+encodeURIComponent(src);
            this.videoPlay = true;
