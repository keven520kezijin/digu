  ...
  <div class="speech-tab-pane">
    <div>
      ...
      <el-row style="padding-bottom:20px;">
        ...
        <el-button type="primary" style="float:right" :disabled="disabled" @click="releaseVisible = true"
          >发布</el-button
        >
      </el-row>
      <el-row>
        ...
        <el-col :span="20">
          ...
          >
            ...
            <el-table-column prop="id" width="240" label="ID"> </el-table-column>
            <el-table-column prop="intentionName" label="意图名称"> </el-table-column>
            <el-table-column
            ...
            <el-table-column sortable prop="createTime" width="180" label="更新时间"> </el-table-column>
            <el-table-column prop="creator" label="操作人" width="180"> </el-table-column>
            <el-table-column width="100" label="操作">
              <template slot-scope="scope">
                <span class="operation-box">
                  ...
                  <el-button
                    type="text"
                    :disabled="scope.row.intentionStatus == 2"
                    @click="handleRelease(scope.row, scope.$index)"
                    >发布</el-button
                  >
                </span>
              </template>
    ...
    <el-dialog class="addQuestion" title="删除意图" :visible.sync="delIntentVisible" width="400px">
      <div>您确定要删除已选中的{{ this.multipleSelection.length }}个意图吗？</div>
    ...
    <el-dialog title="导入新增意图" :visible.sync="importEntenVisible" width="400px">
      <div>本次导入将新增{{ importFileLength }}个意图，确定继续导入吗？</div>
      <span slot="footer" class="dialog-footer">
      ...
      <div>
        <el-row class="clearfix">
          ...
          <el-col :span="17" class="fileCol">
            ...
            <el-input type="text" :value="fileName"> </el-input>
          ...
          <el-form class="clearfix" ref="importForm" :model="importForm" :rules="importFormRules" label-width="100px">
            <el-form-item label="行业领域：" prop="industryType">
              <el-select
                v-model="importForm.industryType"
                placeholder="请选择行业领域"
                @change="changeValue"
                style="width:326px;"
              >
                <el-option v-for="item in selectOpt" :key="item.id" :label="item.industryName" :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
          </el-form>
    ...
    <el-dialog title="批量发布意图" :visible.sync="releaseVisible" width="400px">
      <div>您确定要发布已选中的{{ this.multipleSelection.length }}个意图吗？</div>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="批量发布意图" :visible.sync="releaseOneVisible" width="400px">
      <div>您确定要发布"{{ this.releaseRow.intentionName }}"吗？</div>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    return {
      ...
      addFormRules: {
        ...
        industryType: [{ required: true, message: '请选择行业领域！' }],
      },
      importForm: {
        intentionName: '',
        industryType: '',
  ...
  methods: {
    ...
    removeIntentionSpeechcraftList() {
      ...
      $.post(this.$baseUrl + 'services/intention/removeIntentionSpeechcraftList.json', JSON.stringify(ids)).then(
        res => {
          if (res.resultMessageEnum == '0000') {
    ...
    createExportIntentionExcelFile() {
      ...
      $.post(this.$baseUrl + 'services/intention/createExportIntentionExcelFile.json', JSON.stringify(params)).then(
        res => {
          if (res.resultMessageEnum == '0000') {
    ...
    addIntentionSpeechcraft() {
      ...
      this.$refs['addForm'].validate(valid => {
        if (valid) {
          $.post(this.$baseUrl + 'services/intention/addIntentionSpeechcraft.json', JSON.stringify(params)).then(
            res => {
              if (res.resultMessageEnum == '0000') {
