<template>
  <div class="speech-tab-pane">
    ...
    <el-tabs :value="cerActiveName" @tab-click="handleTabClick">
    ...
    <div>
        ...
        <el-col :offset="2" :span="20" style="text-align:right;">
          <el-form :inline="true" :model="searchForm" class="demo-form-inline">
            <el-form-item label="审核状态" v-show="cerActiveName == '待处理'">
              <el-select @change="typeChange" v-model="searchForm.speechcraftStatusList" placeholder="审核状态">
      ...
      <el-table :data="certiFicationTableData" @sort-change="sortChange" style="width: 100%">
        <el-table-column prop="speechcraftName" label="话术名称"> </el-table-column>
        <el-table-column prop="speechcraftStatus" label="审核状态" :formatter="formatterSpeechcraftStatus" width="180">
        </el-table-column>
        <el-table-column
        ...
        <el-table-column width="180" label="操作">
          <template slot-scope="scope">
            <span>
              <el-button
                type="text"
                v-if="scope.row.speechcraftStatus == 2"
                @click="handleView(scope.$index, scope.row)"
                >审核</el-button
              >
              <el-button
                type="text"
                v-else-if="scope.row.speechcraftStatus == 7"
                @click="handleView(scope.$index, scope.row)"
                >重审</el-button
              >
              <el-button type="text" v-else @click="handleView(scope.$index, scope.row)">查看</el-button>
              <el-button type="text" v-if="cerActiveName !== 'first'" @click="handleBert(scope.row)"
                >bert 模型</el-button
              >
            </span>
          </template>
...
export default {
  ...
  computed: {
    ...mapState({
      cerActiveName: state => state.speach.cerActiveName
    })
  },
  mounted() {
    //  是否显示tab
  ...
  methods: {
    ...
    handleTabClick(tab) {
      this.setCerActiveName(tab.name)
      if (tab.name == 'first') {
        // this.cerStatusTxt = '待处理';
        this.speechcraftStatusList = ['2', '4', '6', '7'];
      } else {
    ...
    handleBert(row) {
      this.$router.push(`/speech/bert/${row.speechcraftId}/${row.nluAppKey}/${row.speechcraftName}`)
    }
  },
  destroyed() {
