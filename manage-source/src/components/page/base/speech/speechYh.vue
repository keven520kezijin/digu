        ...
        <div>
          ...
          <el-row>
            ...
            <el-col :span="20">
              ...
              >
                <el-table-column prop="speechcraftName" label="话术名称"> </el-table-column>
    ...
    <el-dialog title="复制到通用话术库" :visible.sync="copyVisible" width="400px">
      <el-form ref="copyForm" @va :rules="copyFormRules" class="copyForm" :model="copyForm" label-width="100px">
        <el-form-item label="话术名称：" prop="newSpeechcraftName">
          <el-input
            @change="handleChange"
            v-model="copyForm.newSpeechcraftName"
            placeholder="请输入话术名称"
          ></el-input>
        </el-form-item>
        <el-form-item label="行业领域：" prop="industryType">
          <el-select v-model="copyForm.industryType" placeholder="请选择行业">
            <el-option v-for="item in validIndustryListData" :key="item.id" :label="item.industryName" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
          ...
          <el-tooltip class="item" effect="light" popper-class="speech-tip" placement="right-start">
            <div slot="content">
              该话术模版关联的音频也会自<br />动复制到【通用音频库】中的<br />【话术音频】文件夹下
            </div>
        ...
        <el-form-item prop="soundConfigGroup" v-if="copyForm.isCopySound">
          <el-select
            ...
            allow-create
            @change="handleChange"
            v-model="copyForm.soundConfigGroup"
            value-key="id"
            placeholder="选择音频存储文件夹"
          >
...
export default {
  ...
  data() {
    return {
      ...
      soundConfigGroup: [],
      copyFormRules: {
        newSpeechcraftName: [{ required: true, message: '话术名称不能为空！' }],
        industryType: [{ required: true, message: '请选择行业领域！' }],
        soundConfigGroup: [{ required: true, message: '选择“同时复制音频”选项时，音频存储文件夹不能为空！' }],
      },
      selectOpt: [],
      pagination: {
        show: true,
  ...
  methods: {
    ...
    handleCopy(row, index) {
      ...
      this.copyForm.soundConfigGroup = {
        id: 'duplicateid',
        configGroupName: this.copyForm.newSpeechcraftName,
    ...
    querySoundConfigGroupList() {
      $.get(
        this.$baseUrl + 'services/sound/querySoundConfigGroupList.json?duplicateFlag=1&soundConfigGroupType=1',
      ).then(res => {
    ...
    handleView(row, index) {
      let url = `${baseHost}/flow/index.html?viewType=0&speechcraftId=${row.speechcraftId}&flowType=1`;
      window.open(url);
    },
            ...
            if (res.resultMessageEnum == '0000') {
              if (this.copyForm.isCopySound) {
                this.$message({
                  ...
                  message:
                    this.copyForm.soundConfigGroup.configGroupName +
                    '音频复制成功，<a target="_blank" href="' +
                    this.$baseHost +
                    '/#/resource">点击查看</a>',
                });
              } else {
