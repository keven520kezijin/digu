    ...
    <div>
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true">
          ...
          <el-form-item label="操作人：" prop="memberIds">
            <el-select
              ...
              :disabled="searchForm.allManager"
              placeholder="请选择"
            >
              <el-option
      ...
      <el-row>
        <el-col :span="24">
          ...
          >
            <el-table-column prop="industryName" label="行业领域"></el-table-column>
              ...
              <template slot-scope="scope">
                <span class="operation-box">
                  ...
                  <el-button
                    ...
                    :disabled="scope.row.dictionaryStatus === 2"
                    type="text"
                    @click="handlePublish(scope.$index, scope.row)"
                  >发布</el-button>
                  <el-button
                    v-else
                    :disabled="scope.row.dictionaryStatus === 4"
                    type="text"
...
export default {
  ...
  data() {
    return {
      ...
      addFormRules: {
        dictionaryName: [{ required: true, message: '词典名称不能为空！' }],
        industryScope: [{ required: true, message: '行业领域不能为空！' }],
      },
      importForm: {
        dictionaryName: '',
        industryScope: '',
      ...
      settingRules: {
        dictionaryName: [{ required: true, message: '词典名称不能为空！', trigger: 'change' }],
      },
  ...
  computed: {
    filteredData() {
      const { queryKey, memberIds } = this.filterParam;
      let filtered = this.validIndustryListData;
      if (queryKey) {
        filtered = filtered.filter(l => this.fuzzysearch(queryKey, l.industryName));
      }
      if (memberIds && memberIds.length) {
        const memberSet = new Set(memberIds);
        filtered = filtered.filter(l => memberSet.has(l.editor));
      }
      return filtered;
    },
    tableData() {
      const currentPage = this.cur_page - 1;
      ...
      if (sortList.length) {
        const { propertyName, ascendingFlag } = sortList[0];
        listData = sortBy(listData, [propertyName]);
        if (!ascendingFlag) {
          listData = listData.reverse();
        }
      }
      return listData.slice(currentPage * pageSize, (currentPage + 1) * pageSize);
    },
    pageTotal() {
  ...
  methods: {
    ...
    handleSearch() {
      ...
      this.filterParam = { ...this.searchForm };
      // this.handleQueryDictionaryList();
    },
