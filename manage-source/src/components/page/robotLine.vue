  ...
  <el-tabs v-model="activeName" @tab-click="handleClick">
      ...
      >
        ...
        <el-form-item label="分配比例" prop="distributionRatio">
          <el-col :key="index" v-for="(item, index) in robotLineConfigVO.robotLineDistributionRatios">
            ...
            <el-input
              type="text"
              v-model="robotLineConfigVO.robotLineDistributionRatios[index].systemValue"
              size="medium"
              style="width:60px"
            >
            </el-input>
    ...
    <el-tab-pane label="任务分配详情" name="second">
      ...
      <el-table :data="taskData" style="width: 100%">
        ...
        <el-table-column prop="maxAllowAgent" label="当前线路数" width="180"> </el-table-column>
      </el-table>
      <el-pagination
        background
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          $.post(
            this.$baseUrl + 'services/config/updateRobotLineConfig.json',
            JSON.stringify(this.robotLineConfigVO),
          ).then(res => {
            if (res.resultMessageEnum == '0000') {
