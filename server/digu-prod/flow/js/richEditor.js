...
richEditor.bindEvent = function(style, isDisabled) {
  ...
  if ($.utils._dialogVue.knowledgeVisible) {
    $('#hungup-speechcraft-content>.' + style).on('DOMNodeInserted DOMNodeRemoved input propertychange', function(e) {
      setTimeout(() => {
  ...
  // 话术编辑器
    ...
    if (!isDisabled) {
      $('.' + style).on('DOMNodeInserted DOMNodeRemoved input propertychange', function(e) {
        $.utils._speechcraftContent[$.utils._flowTypeName].forEach(elem => {
          if (elem.name === $.utils._speechEditableTabsName) {
            setTimeout(() => {
