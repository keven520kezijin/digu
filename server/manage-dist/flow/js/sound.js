		...
		setActiveGroupType(type){
         ...
         handleVideo(index,row){
        	 ...
        	 this.$nextTick(() => {
        		 ...
        		 this.viewSource = _baseUrl + '/services/file/downloadFile.html?filePath=' + encodeURIComponent(row.soundPath);
             });
         },
         //得到文件播放时长
         getFilePlayDuration(){
             return $.get(_baseUrl + '/services/file/getFilePlayDuration.json?filePath=' + encodeURIComponent(this.soundPath)).then((res)=>{
                 if(res.resultMessageEnum=='0000'){
                     return res;
                 }else if(res.resultMessageEnum == "0001"){
                	 soundVue.$message.error(winOpener.$.utils.getErrorMessage(res.errorInfoList));
